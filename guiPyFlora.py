...
class guiFlora():
    def __init__(self, r) -> None:
        ...
        # icons
        self.logo_img = ImageTk.PhotoImage(self.logo_open.resize((150, 150), Image.LANCZOS))
        self.bg_logo = ImageTk.PhotoImage(self.logo_bg_open.resize((100, 150), Image.LANCZOS))
        self.avatar_img = ImageTk.PhotoImage(self.avatar_open.resize((40, 30), Image.LANCZOS))
        self.biljke_naslov_img = ImageTk.PhotoImage(self.biljke_naslov.resize((150, 40), Image.LANCZOS))
        self.mojprofil_naslov_img = ImageTk.PhotoImage(self.mojprofil_naslov.resize((170, 40), Image.LANCZOS))
        self.pyposude_naslov_img = ImageTk.PhotoImage(self.pyposude_naslov.resize((170, 40), Image.LANCZOS))
        self.add_new_img = ImageTk.PhotoImage(self.add_new.resize((50, 50), Image.LANCZOS))
        self.img_watering = ImageTk.PhotoImage(self.watering.resize((35, 30), Image.LANCZOS))
        self.img_suplement = ImageTk.PhotoImage(self.suplement.resize((40, 40), Image.LANCZOS))
        self.img_sunlight = ImageTk.PhotoImage(self.sunlight2.resize((40, 40), Image.LANCZOS))
        self.img_reduce_temperature = ImageTk.PhotoImage(self.reduce_temperature.resize((30, 30), Image.LANCZOS))
        self.img_reduce_sunlight = ImageTk.PhotoImage(self.reduce_sunlight2.resize((40, 40), Image.LANCZOS))
        self.img_add_temperature = ImageTk.PhotoImage(self.add_temperature.resize((25, 30), Image.LANCZOS))
        self.img_ok = ImageTk.PhotoImage(self.status_ok.resize((30, 30), Image.LANCZOS))
        self.img_empty = ImageTk.PhotoImage(self.empty_pot.resize((100, 100), Image.LANCZOS))
...
class Admin(guiFlora):
    ...
    def view_list(self, database, table, frame):
        ...
        for row in rows:
            ...
            if table == self.tb_biljke:
                photo = row[1]
                if type(photo) == str:
                    img_opn = Image.open(photo).resize((90, 120), Image.LANCZOS)
                else:
                    stream = BytesIO(photo)
                    img_opn = Image.open(stream).resize((90, 120), Image.LANCZOS)
    ...
    def upload_img(self): 
            ...
            if self.filename:
                ...
                self.img_open = Image.open(self.img_path).resize((90, 120), Image.LANCZOS)
    ...
    def plant_update(self, nm_pl):
        ...
        for pl in plants:
            if type(pl[1]) == str:
                ...
                img_opn = Image.open(img_path).resize((90, 120), Image.LANCZOS)
            else:
                stream = BytesIO(pl[1])
            ...
            if has_plant == 'True' or has_plant == True or has_plant == 1:
                ...
                for o in ocitanja:
                    ...
                    if type(take_photo) == str:
                        img_opn = Image.open(take_photo).resize((60, 90), Image.LANCZOS)
                    else:
                        photo = BytesIO(take_photo)
                        img_opn = Image.open(photo).resize((60, 90), Image.LANCZOS)
                    img_b = ImageTk.PhotoImage(img_opn)
